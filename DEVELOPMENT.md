...
Someone who is in the Fedora *packager* group can create Fedora and
EPEL builds.  The process is:

1. `kinit username@FEDORAPROJECT.ORG` (required for SPNEGO HTTP
   authentication for various steps).
2. Clone Fedora/EPEL dist-git: `fedpkg clone ipa-hcc && cd ipa-hcc`
3. Update `ipa-hcc.spec` using the Fedora-specific specfile
   generated by `make build/fedora/ipa-hcc.spec`.
4. Download new source tarball: `spectool --get-files ipa-hcc.spec`
5. Upload new sources and update `sources` file:
   `fedpkg new-sources ipa-hcc-$VERSION-$RELEASE.tar.gz ...`.
   This also updates the `.gitignore` and `sources` files.  Be sure
   to include **all** source and patch files.
6. Commit the updated `ipa-hcc.spec`, `sources` and `.gitignore` files.
7. Generate `.src.rpm` by running `fedpkg srpm`
8. Run `fedpkg lint` to check for any issues.
9. Do a Koji scratch build: `koji build --scratch <TARGET> <SRPM>`,
   where `TARGET` is e.g. `f41`, `epel9`, etc.
10. If all is well, run `fedpkg push`, then `fedpkg build` to start
    an official build.
11. Push changes and perform builds for branched releases.  See
    [Working with branches].
12. Create updates for branched releases where *updates-testing* is
    active: `fedpkg update`.  See [Package Update Guide] for more
    info.

[Working with branches]: https://docs.fedoraproject.org/en-US/package-maintainers/Package_Maintenance_Guide/#working_with_branches
[Package Update Guide]: https://docs.fedoraproject.org/en-US/package-maintainers/Package_Update_Guide/#later_branched_and_stable_releases
